...
export const Map = (props: any) => {
  ...
  return (
    <>
      ...
      <TouchableOpacity style={{ elevation: 5, top: Platform.OS === 'android' ? StatusBar.currentHeight : area.top }}
        className="absolute flex items-center bg-white p-3 m-3 rounded-lg"
        onPress={() => props.navigation.goBack()}>
        <Image className="h-3 w-3" source={require('../../assets/x.png')} />
      </TouchableOpacity>
      <View>
      </View>
      <TouchableOpacity className="absolute self-end flex items-center bg-white p-3 bottom-7 right-7 rounded-full" style={{ elevation: 3 }} onPress={_onCenter}>
        <Image className="h-6 w-6 rotate-90" source={require('../../assets/center-of-gravity.png')} />
      </TouchableOpacity>
    </>
  )
}
